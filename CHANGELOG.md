...
+ [新增] 支持通过 USA零网站账号授权选择已有 API Key，并以只读网站渠道同步可用的文本、图片和视频模型。
+ [新增] 本地 Agent 连接页支持下载带跨平台安装脚本的完整 USA零 Codex 插件包。
+ [修复] USA零账号授权改由网站同源创建请求，避免本地画布登录弹窗因跨域预检失败而立即关闭，并复用网站已有登录态。
+ [修复] 画布本地图片或媒体恢复异常时不再永久停留在加载骨架屏。

+ [调整] 全站品牌统一为 USA零，替换 Logo、官网、Q群、仓库与版本更新来源，并清理旧宣传入口。
+ [调整] Codex 插件默认改为打开本地 3000 端口的 USA零 画布。
+ [修复] Canvas Agent 在 npx 缓存缺少 Codex 平台包元数据时直接启动原生程序，避免新建会话后初始化失败。
...
+ [新增] 新增用户账号与算力点体系，支持账号密码注册登录与第三方 OAuth。
...
+ [新增] 首次开源版本，包含USA零能力：多画布项目、节点拖拽缩放、连线、小地图、撤销重做、导入导出。
