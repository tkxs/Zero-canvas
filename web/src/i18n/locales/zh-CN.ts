export default {
    ...
    settingsPanels: {
        ...
        model: { select: "选择模型", assign: "请先为{{capability}}选择一个 Key 模型", noMatch: "暂无匹配的{{capability}}模型", addFirst: "请先到账户管理同步或创建可用 Key", capabilities: { image: "生图", video: "视频", text: "文本", audio: "音频" } },
    },
    ...
    apiErrors: { requestFailed: "请求失败", requestCanceled: "请求已取消", baseUrlRequired: "当前官网 Key 的模型网关地址不可用，请到账户管理同步 Key", apiKeyRequired: "当前模型对应的官网 Key 已删除或不可用，请到账户管理同步或创建 Key", authenticationFailed: "鉴权失败，请检查官网 Key 状态、套餐权限或模型权限", rateLimited: "请求被限流或额度不足，请稍后重试", notFound: "模型接口不存在（404），请检查官网 Key 分组和模型选择", badGateway: "网关错误（502），模型服务暂时不可用，请稍后重试", serviceBusy: "服务繁忙（503），请稍后重试", httpFailed: "请求失败（HTTP {{status}}），请检查官网 Key 状态和模型权限", htmlError: "服务返回了 HTML 错误页面（{{preview}}）", audioModelRequired: "请先选择音频模型", audioGenerationFailed: "音频生成失败", geminiAudioUnsupported: "该 Gemini 调用格式暂不支持音频生成，请选择支持音频的其他 Key 模型", invalidImageSizeFormat: "图像尺寸格式不支持，请使用 auto、9:16 或 1024x1024", positiveImageRatio: "图像比例必须是正数，例如 9:16", imageRatioLimit: "图像宽高比不能超过 3:1，请调整尺寸", positiveImageDimensions: "图像尺寸必须是正整数，例如 1024x1024", imageDimensionStep: "图像尺寸的宽高必须是 16 的倍数，请调整尺寸", imageEdgeLimit: "图像尺寸最长边不能超过 3840px，请调整尺寸", imagePixelLimit: "图像总像素需在 655360 到 8294400 之间，请调整尺寸", unknownImageResponse: "接口返回了未知格式的数据（字段：{{fields}}），请检查模型或接口兼容性", noImageReturned: "接口没有返回图片，请检查提示词是否触发安全审核或模型是否支持该操作", geminiRejected: "Gemini 拒绝了本次请求：{{reason}}", geminiNoImage: "Gemini 接口没有返回图片", geminiMaskUnsupported: "Gemini 调用格式暂不支持蒙版编辑", maskModelUnsupported: "蒙版编辑暂不支持该模型，请选择其他 Key 模型", noContent: "没有返回内容", modelReadFailed: "读取模型失败", videoTimeout: "{{provider}}视频生成超时，请稍后重试", videoReferencesUnsupported: "当前视频接口不支持参考视频或参考音频，请切换到 Seedance 2.0 / 火山 Agent Plan 模型，或移除参考资产", noPlayableVideo: "视频接口没有返回可播放的视频", noVideoTaskId: "视频接口没有返回任务 ID", videoTaskCreateFailed: "视频任务创建失败", videoGenerationFailed: "视频生成失败", videoTaskQueryFailed: "视频任务查询失败", seedanceAudioRequiresVisual: "Seedance 参考音频不能单独使用，请同时添加参考图或参考视频", videoPromptRequired: "请输入视频提示词，或连接参考图片/视频/音频", seedanceNoTaskId: "Seedance 接口没有返回任务 ID", seedanceTaskCreateFailed: "Seedance 任务创建失败", seedanceNoVideoUrl: "Seedance 任务成功但没有返回视频 URL", seedanceVideoTimeout: "Seedance 视频生成超时", seedanceVideoFailed: "Seedance 视频生成失败", seedanceTaskQueryFailed: "Seedance 任务查询失败", seedanceVideoDuration: "Seedance 参考视频单个时长需要在 2-15 秒之间", seedanceVideoTotalDuration: "Seedance 参考视频总时长不能超过 15 秒", seedanceAudioDuration: "Seedance 参考音频单个时长需要在 2-15 秒之间", seedanceAudioTotalDuration: "Seedance 参考音频总时长不能超过 15 秒", referenceImageReadFailed: "参考图读取失败，请换一张图片或重新上传", invalidReferenceVideo: "参考视频必须是公网 URL、资产 ID，或本地已保存的视频", invalidReferenceAudio: "参考音频必须是公网 URL、资产 ID，或本地已保存的音频", videoModelRequired: "请先选择视频模型", geminiVideoUnsupported: "该 Gemini 调用格式暂不支持视频生成，请选择支持视频的其他 Key 模型", noVideoTask: "接口没有返回视频任务", seedanceNoTask: "Seedance 接口没有返回任务", videoDownloadFailed: "视频下载失败", localAssetReadFailed: "读取本地资产失败" },
    prompts: {
        title: "提示词中心",
        library: "提示词库",
    ...
    canvas: {
        ...
        projectPage: {
            localMediaRestoreFailed: "部分本地素材读取失败，已使用保存的数据打开画布",
        ...
        plugins: { title: "节点插件", installedPlugin: "已安装插件 {{name}}", installed: "已安装 {{name}}", installFailed: "安装失败：{{error}}", enabled: "已启用", disabled: "已禁用", upgradeAvailable: "有新版本，点击升级", updateFromSource: "从来源更新", updated: "已更新", uninstallTitle: "卸载该插件？", uninstall: "卸载", newVersion: "有新版本可升级", officialDescription: "本项目官方插件，来自仓库注册表", refresh: "刷新", loadFailed: "加载失败：{{error}}", loadingOfficial: "正在获取官方插件…", noOfficial: "暂无官方插件", install: "安装", urlPlaceholder: "输入插件 JS 文件 URL，例如 https://.../plugin.js", noThirdParty: "还没有安装第三方插件", official: "官方插件", local: "本地插件", thirdParty: "第三方插件", warning: "插件代码会在当前页面内直接执行并可能访问页面本地数据，请仅安装可信来源。插件 SDK 不会提供官方完整 Key。", aiConfigRequired: "AI 模型尚不可用，请先到账户管理同步或创建官网 Key", interactiveTitle: "当前：交互中。点击切回「移动」——拖动可移动节点", movableTitle: "当前：可移动。点击切到「交互」——可操作节点内容（如转动全景）", move: "移动", interact: "交互" },
    ...
    topNav: {
        officialSite: "官网",
        qqGroup: "复制 Q 群号：1084450051",
        qqGroupCopied: "Q 群号已复制",
        openMenu: "打开导航菜单",
        menu: "导航菜单",
        navigation: "导航",
        openAgent: "打开 Agent",
    ...
    account: {
        title: "USA零账号",
        subtitle: "登录认证与官网 Key 管理",
        connectTitle: "连接网站账号",
        connectDescription: "登录将在 USA零官网授权窗口中完成；如果官网已有登录状态，可直接继续授权，未注册用户可在官网完成注册。",
        login: "登录或注册并授权",
        openWebsite: "打开 USA零官网",
        connected: "已连接",
        needsKey: "需要 Key",
        syncIssue: "同步异常",
        keyTotal: "官网 Key",
        modelTotal: "同步模型",
        keysTitle: "密钥 Key",
        keysDescription: "所有 Key 与官网实时同步，每个可用 Key 都会独立提供其分组下的模型。",
        syncKeys: "同步 Key",
        keysSynced: "Key 已与官网同步",
        createKeyTitle: "创建官网 Key",
        createKeyDescription: "选择 Key 所属分组，确认后将同时在官网创建。",
        createGroup: "选择官网分组",
        selectGroup: "选择一个分组",
        createKey: "创建 Key",
        keyCreated: "Key 已在官网创建并同步",
        deleteKeyTitle: "删除官网 Key",
        deleteKeyConfirm: "确定删除“{{name}}”吗？官网中的同一 Key 也会被删除。",
        keyDeleted: "Key 已从官网删除",
        noKeysCreate: "当前没有 Key，请选择分组创建",
        noGroups: "当前没有可用分组，请先在官网配置分组",
        noGroup: "未绑定分组",
        modelCount: "{{count}} 个模型",
        syncFailed: "同步失败",
        viewUsage: "查看用量",
        usageSummary: "余额 {{balance}} · 今日 Token {{today}} · 累计 Token {{total}}",
        keySourceUnavailable: "该模型对应的官网 Key 已删除或不可用，请重新选择模型",
        modelCapabilities: { text: "文本", image: "图片", video: "视频", audio: "音频" },
        logout: "退出登录",
        keyReasons: { disabled: "已停用", ungrouped: "未绑定分组", expired: "已过期", quotaExhausted: "配额已耗尽" },
        progress: {
            restoring: "正在恢复账号会话...",
            authorizing: "等待 USA零官网完成授权...",
            syncing: "正在同步账号、Key 与模型...",
        },
        summary: {
            restoring: "正在恢复账号信息...",
            notSignedIn: "登录后自动同步官网 Key、模型和用量信息。",
            userId: "用户 ID",
            apiKeyName: "查看中的 Key",
            noSelectedKey: "暂无可用 Key",
            group: "所属分组",
            none: "无",
            keyStatus: "Key 状态",
            available: "可用",
            ...
            unknown: "未知",
            syncedModels: "同步模型",
            currentKeyUsage: "Key 用量",
            inspectedKeyUsage: "{{name}} 用量",
            balance: "可用余额",
            todayTokens: "今日 Token",
            totalTokens: "累计 Token",
            quotaUsage: "额度使用",
            refreshUsage: "刷新用量",
            usageUnavailable: "暂时无法获取查看中 Key 的用量。",
            usageStale: "暂时无法获取最新用量，已保留上次数据。",
            logoutFailed: "退出登录失败，请重试。",
            updatedAt: "更新于 {{time}}",
            selectKeyHint: "请到账户管理中创建或同步可用 Key。",
    ...
    config: {
        ...
        description: "默认模型、生成偏好、同步与本地存储",
        modalDescription: "用户偏好、提示词来源、同步与本地存储",
        tabs: {
            preferences: "偏好设置",
            promptSources: "提示词来源",
            localStorage: "本地存储",
        ...
        localStorage: {
            ...
            description: "查看 USA零 在浏览器中保存的数据量，并按对象仓库统计内容体积。",
        ...
        fileSecurity: "官网 Key 不会写入配置文件；JSON 仍可能包含 WebDAV 凭据，请妥善保管。",
        import: "导入配置",
        export: "导出配置",
        imported: "配置与用户偏好已导入",
    ...
    agent: {
        ...
        connect: { pluginTitle: "方式一：在 Codex 中使用插件", pluginText: "下载完整 USA零 Codex 插件，安装后由插件自动启动本地 Agent 并带上连接信息。", pluginContents: "完整插件 · 2 个 Skill + Canvas Agent MCP", downloadPlugin: "下载 Codex 插件", pluginDownloadFailed: "Codex 插件下载失败，请确认当前站点已包含插件下载包", installStepDownload: "下载 ZIP 并解压到任意临时目录。", installStepRun: "在解压目录中运行当前系统的安装命令。", installStepRestart: "安装完成后新建 Codex 任务，并输入“帮我打开并连接到 USA零”。", manualInstall: "查看手动安装命令", installUpdateHint: "重复运行安装脚本即可更新；安装或更新后需要新建 Codex 任务才能加载新的 Skill 和 MCP。", directTitle: "方式二：直接运行 Agent", directText: "不安装 Codex 画布工具时，可在终端直接运行 Agent，再回到网页连接；这种方式不会向 Codex 添加 Skill 或 MCP。", commandCopied: "命令已复制", pluginReminder: "Codex 插件提醒", pluginReminderText: "只有安装 Codex 插件或手动添加 MCP 后，工具列表才会进入 Codex 上下文并增加 token 消耗；仅运行 npx -y @basketikun/canvas-agent 启动本地 Agent 不会安装 MCP。", removePlugin: "移除插件", removeMcp: "移除手动 MCP", copyCommand: "复制命令", title: "连接本地 Agent", description: "按使用场景选择一种连接方式。", webConnection: "网页连接", autoDiscover: "默认自动读取 Local URL 和 Connect token，失败时再手动填写。", disconnect: "断开", connect: "连接", localAddress: "本地地址", urlPlaceholder: "例如 http://127.0.0.1:17371", token: "连接 Token", tokenPlaceholder: "自动发现，或手动填入 Connect token" },
