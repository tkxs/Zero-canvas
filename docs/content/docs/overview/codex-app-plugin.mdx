...
description: Install and use the USA零 plugin in Codex App
---

# Codex App Plugin

The plugin connects Codex to a local Canvas Agent, allowing Codex to open USA零, read and create nodes, connect nodes, and start generation workflows. Its source is in `plugins/infinite-canvas/`.
...
```text
Install the USA零 Codex plugin from https://github.com/tkxs/Zero-canvas.git.
...
```bash
mkdir -p ~/plugins
git clone https://github.com/tkxs/Zero-canvas.git ~/plugins/infinite-canvas
...
In a new Codex conversation, ask it to open USA零, create a canvas, and connect Codex. The plugin starts Canvas Agent, reads its connection URL and token, opens `https://usa0.top/`, and creates and connects a canvas. It starts a local frontend only when explicitly requested.
